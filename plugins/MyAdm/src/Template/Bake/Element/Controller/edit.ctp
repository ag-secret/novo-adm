    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->is(['patch', 'post', 'put'])) {
            ...
            if ($this-><%= $currentModelName; %>->save($<%= $singularName %>)) {
                $this->Flash->success('O <%= strtolower($singularHumanName) %> foi salvo.');
                return $this->redirect(['action' => 'index']);
            } else {
                $this->Flash->error('O <%= strtolower($singularHumanName) %> não pode ser salvo. Por favor, tente novamente.');
            }
        }
<%
